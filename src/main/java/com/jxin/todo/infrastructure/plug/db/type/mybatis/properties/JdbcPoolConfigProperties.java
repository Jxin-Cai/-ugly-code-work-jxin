 ...
 */
@Data
public class JdbcPoolConfigProperties {
    /**
     * 此属性控制从池返回的连接的默认自动提交行为。它是一个布尔值。
     * 默认值：true
     */
    private boolean autoCommit = HikariCpDefaultConfigConsts.DEFAULT_AUTO_COMMIT;
    /**
     ...
     * 默认值：30000（30秒）
     */
    private int connectionTimeout = HikariCpDefaultConfigConsts.DEFAULT_CONNECTION_TIMEOUT;
    /**
     * 此属性控制允许连接在池中闲置的最长时间。
     ...
     * 默认值：600000（10分钟）
     */
    private int idleTimeout = HikariCpDefaultConfigConsts.DEFAULT_IDLE_TIMEOUT;
    /**
     ...
     * 默认值：1800000（30分钟）
     */
    private int maxLifetime = HikariCpDefaultConfigConsts.DEFAULT_MAX_LIFETIME;
    /**
     ...
     * 默认值：10
     */
    private int maximumPoolSize = HikariCpDefaultConfigConsts.DEFAULT_MAXIMUM_POOLSIZE;
    /**
     ...
     */
    private int minimumIdle = HikariCpDefaultConfigConsts.DEFAULT_MINIMUM_IDLE;
    /**
     ...
     * 默认值：1
     */
    private int initializationFailTimeout = HikariCpDefaultConfigConsts.DEFAULT_INITIALIZATION_FAIL_TIMEOUT;
    /**
     * 如果池无法成功初始化连接,则此属性控制池是否将“快速失败”。
     * 任何正数都取为尝试获取初始连接的毫秒数; 应用程序线程将在此期间被阻止。
     * 如果在超时发生之前无法获取连接,则会引发异常。
     ...
     * 默认值：1
     */
    private boolean isolateInternalQueries = HikariCpDefaultConfigConsts.DEFAULT_ISOLATE_INTERNAL_QUERIES;
    /**
     * 该属性控制池是否可以通过JMX暂停和恢复。这对于某些故障转移自动化方案很有用。
     * 当池被暂停时,呼叫 getConnection()将不会超时,并将一直保持到池恢复为止。
     * 默认值：false
     */
    private boolean allowPoolSuspension = HikariCpDefaultConfigConsts.DEFAULT_ALLOW_POOL_SUSPENSION;
    /**
     ...
     * 默认值：false
     */
    private boolean readOnly = HikariCpDefaultConfigConsts.DEFAULT_READ_ONLY;
    /**
     * 该属性控制是否注册JMX管理Bean（“MBeans”）。
     * 默认值：false
     */
    private boolean registerMbeans = HikariCpDefaultConfigConsts.DEFAULT_REGISTER_MBEANS;
    /**
     ...
     * 默认值：无
     */
    private String driverClassName = HikariCpDefaultConfigConsts.DEFAULT_DRIVER_CLASS_NAME;
    /**
     * 此属性控制连接测试活动的最长时间。这个值必须小于connectionTimeout。最低可接受的验证超时时间为250 ms。
     * 默认值：5000
     */
    private int validationTimeout = HikariCpDefaultConfigConsts.DEFAULT_VALIDATION_TIMEOUT;
    /**
     * 此属性控制在记录消息之前连接可能离开池的时间量,表明可能存在连接泄漏。值为0意味着泄漏检测被禁用。
     * 启用泄漏检测的最低可接受值为2000（2秒）。
     * 默认值：0
     */
    private int leakDetectionThreshold = HikariCpDefaultConfigConsts.DEFAULT_LEAK_DETECTION_THRESHOLD;
}
